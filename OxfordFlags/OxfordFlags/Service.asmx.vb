...
Public Class Service
    ...
    <WebMethod()> _
    <ScriptMethod(ResponseFormat:=ResponseFormat.Json)>
    Public Function FindExistingBuyers(ByVal input As String) As String()
        Dim buyers As New List(Of String)()
        Using conn As New SqlConnection()
            conn.ConnectionString = ConfigurationManager.ConnectionStrings("ConnectionString").ConnectionString
            Using cmd As New SqlCommand()
                cmd.CommandText = "SELECT FirstName, LastName, BuyerID, Active FROM Buyer where " & "Active = '1' AND (FirstName like @SearchText + '%' OR LastName like @SearchText +'%')"
    ...
    <WebMethod()> _
    <ScriptMethod(ResponseFormat:=ResponseFormat.Json)>
    Public Function FindExistingProperties(ByVal input As String) As String()
        Dim properties As New List(Of String)()
        Using conn As New SqlConnection()
            conn.ConnectionString = ConfigurationManager.ConnectionStrings("ConnectionString").ConnectionString
            Using cmd As New SqlCommand()
                cmd.CommandText = "SELECT PropertyOwnerID, StreetAddress, City, State, Zip FROM PropertyOwner where " & "StreetAddress like @SearchText + '%'"
                cmd.Parameters.AddWithValue("@SearchText", input)
                cmd.Connection = conn
                conn.Open()
                Using sdr As SqlDataReader = cmd.ExecuteReader()
                    While sdr.Read()
                        Dim Key As String = sdr("StreetAddress") + " " + sdr("City") + ", " + sdr("State") + " " + sdr("Zip")
                        properties.Add(String.Format("{0}-{1}", Key, sdr("PropertyOwnerID")))
                    End While
                End Using
                conn.Close()
            End Using
